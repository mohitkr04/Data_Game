...
def generate_all_datasets():
    print("Generating enterprise raw datasets...")
    ...
    for i, wh_id in enumerate(warehouse_ids):
        ...
        proc_time = round(random.uniform(2.1, 8.5), 2)
        util_rate = round(random.uniform(62.0, 96.5), 2)
        wh_ops_list.append({
            'warehouse_id': wh_id,
    ...
    for i in range(1, NUM_SUPPORT_TICKETS + 1):
        ...
        if issue in ['Late Delivery', 'Damaged Item']:
            res_time = round(float(np.random.uniform(12.0, 72.0)), 1)
    ...
    print("Data generation complete. All datasets saved to data/raw/")
